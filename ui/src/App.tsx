// Force rebuild: v3.0.3 - Wider chat + Fullscreen mode - CLEAN BUILD
import React, { useEffect, useState } from 'react';
import { Upload, Send, FileText, Trash2, CheckCircle2, AlertTriangle, Loader2, BookOpen, X, Maximize2, Minimize2 } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className='min-h-screen bg-gradient-to-b from-zinc-50 to-zinc-100'>
      ...
      <header className='sticky top-0 z-10 border-b border-zinc-200 bg-white/80 backdrop-blur'>
        <div className='max-w-6xl mx-auto px-4 py-3 flex items-center justify-between'>
          <div className='flex items-center gap-2'>
            ...
            <h1 className='text-lg font-semibold'>Ashley Finance Invoice RAG</h1>
          </div>
      ...
      <main className={isFullscreen ? 'max-w-full mx-auto px-4 py-6' : 'max-w-7xl mx-auto px-4 py-6 grid md:grid-cols-4 gap-6'}>
        {/* Upload Section */}
        {!isFullscreen && (
        ...
        {/* Chat Section */}
        <section className={isFullscreen ? 'w-full' : 'md:col-span-3'}>
            ...
            <div className='flex-1 overflow-auto p-4 space-y-4'>
              ...
              {chat.map((m, i) => (
                <div key={m.ts + i} className={m.role === 'user' ? 'text-right' : 'text-left'}>
                  <div className={
                    `inline-block ${isFullscreen ? 'max-w-full' : 'max-w-[85%]'} px-4 py-3 rounded-2xl border ` +
                    (m.role === 'user'
                    ...
                    <div className='mt-3 text-xs'>
                      <div className='inline-block text-left max-w-full'>
                        ...
                        <div className='flex flex-wrap gap-2'>
                          {(() => {
                            // Remove duplicates by source filename only
                            const uniqueSources = Array.from(
                              new Set(m.sources.map(s => s.source))
                            );

                            return uniqueSources.map((filename, idx) => {
                              return (
                                <span
                                  key={idx}
