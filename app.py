...
# Invoice Intelligence Modules (NEW)
...
# Cosmos DB Store - Dual Mode (Old + New)
from cosmos_store import CosmosVectorStore  # Old store (backward compatibility)
from cosmos_store_new import InvoiceCosmosStore  # New store (invoice intelligence)
...
# Global components - Dual Mode (Old + New)
cosmos_store: Optional[CosmosVectorStore] = None  # Old store (backward compatibility)
invoice_store: Optional[InvoiceCosmosStore] = None  # New store (invoice intelligence)
...
async def startup_event():
    """Initialize all components - Dual Mode (Old + New)"""
    global cosmos_store, invoice_store, hybrid_retriever, embeddings, llm, executor
    ...
    try:
        ...
        # Initialize Old Cosmos Store (backward compatibility)
        cosmos_store = CosmosVectorStore()

        # Initialize New Invoice Store (invoice intelligence)
        try:
            invoice_store = InvoiceCosmosStore()
            logger.info("   🏗️  Invoice Intelligence: Multi-container structure enabled")
        except Exception as e:
            logger.warning(f"   ⚠️  Invoice store initialization failed (non-critical): {e}")
            invoice_store = None

        # Initialize Hybrid Retriever (uses old store)
...
async def process_file_background(file_bytes: bytes, filename: str, file_id: str):
    ...
    try:
        ...
        # ✅ NEW: Extract structured invoice data (if PDF and invoice_store available)
        if invoice_store and filename.lower().endswith('.pdf'):
            try:
                update_status("analyzing", 20, "Extracting invoice fields...")
                ...
                # Save invoice document to invoice_documents container
                invoice_store.save_invoice_document(invoice_doc)

                logger.info(f"✅ Invoice extracted: {invoice_doc.invoice.invoice_number or 'N/A'}, "
                           f"Vendor: {invoice_doc.vendor.name or 'N/A'}, "
                           f"Total: ${invoice_doc.invoice.amounts.total or 0}")

                # Create invoice chunks
                update_status("chunking_invoice", 25, "Creating invoice chunks...")
                ...
                if invoice_chunks:
                    update_status("embedding_invoice", 27, f"Generating embeddings for {len(invoice_chunks)} invoice chunks...")
                    ...
                    logger.info(f"✅ Saved {len(invoice_chunks)} invoice chunks to invoice_chunks container")

                if invoice_doc.flags.needs_review:
                    logger.warning(f"⚠️  Invoice needs review: {filename}")
...
# ============================================================
# NEW INVOICE INTELLIGENCE ENDPOINTS
# ============================================================
...
async def api_get_invoices(
    ...
    if not invoice_store:
        raise HTTPException(
            status_code=503,
            detail="Invoice Intelligence not available. Upload some invoices first."
        )

    try:
        filters = {}
        if invoice_number:
            filters["invoice_numbers"] = [invoice_number.upper()]
        if vendor:
            ...
            "invoices": [
                {
                    ...
                    "flags": {
                        "duplicate": doc.flags.duplicate_candidate,
...
async def api_get_invoice_detail(invoice_number: str):
    """Get detailed information for a specific invoice"""
    if not invoice_store:
        raise HTTPException(status_code=503, detail="Invoice Intelligence not available")
    ...
    try:
        ...
        return {
            "invoice": {
                "number": doc.invoice.invoice_number,
                "date": doc.invoice.invoice_date.isoformat() if doc.invoice.invoice_date else None,
                "po_number": doc.invoice.po_number,
                "amounts": {
                    "subtotal": float(doc.invoice.amounts.subtotal) if doc.invoice.amounts.subtotal else None,
                    "tax": float(doc.invoice.amounts.tax) if doc.invoice.amounts.tax else None,
                    "total": float(doc.invoice.amounts.total) if doc.invoice.amounts.total else None
                }
            },
            "vendor": {
